...
Every one of those is a throughput technique that costs a little latency, which is the correct trade for a system whose job is moving very large volumes of events. Chapter 53 covers the details, and Chapter 131 compares it with RabbitMQ, whose different design targets a different point on the same trade-off.
